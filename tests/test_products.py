...
async def test_create_product(
    client: AsyncClient, sample_category_data: dict, sample_product_data: dict,
) -> None:
...
async def test_list_products_with_filters(
    ...
    response = await client.get("/api/v1/products/?min_price=20&max_price=50")
    assert response.status_code == 200
    data = response.json()
    assert len(data["products"]) >= 1

    # Test filtering by name
    response = await client.get("/api/v1/products/?name=Test")
    assert response.status_code == 200
    data = response.json()
    assert len(data["products"]) >= 1
...
@pytest.mark.asyncio
async def test_invalid_price_range(client: AsyncClient) -> None:
